...
struct CondonHall: UIViewRepresentable {
    class Coordinator {
    ...
    func makeUIView(context: Context) -> MKMapView {
        ...
        let coordinate = CLLocationCoordinate2D(latitude: 44.045097, longitude: -123.078188)
        let region = MKCoordinateRegion(center: coordinate, latitudinalMeters: 800, longitudinalMeters: 500)
        mapView.setRegion(region, animated: true)
        
        let camera = MKMapCamera(lookingAtCenter: coordinate, fromDistance: 800, pitch: 80, heading: 100)
...
struct CondonHallDetailView: View {
    var body: some View {
        ScrollView {
            // Embedding the map
            CondonHall()
